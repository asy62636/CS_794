...
class GPT(nn.Module):
    ...
    def forward(self, idx, past_kvs = None, targets=None, position_offset = 0):
        device = idx.device
        b, t = idx.size()
        ...
        if position_offset > 0:
            # Radix tree mode: use explicit position offset
            pos = torch.arange(position_offset, position_offset + t, dtype=torch.long, device=device)
            if self.debug:
                print(f"[GPT FORWARD] Using position_offset={position_offset}, pos={pos.tolist()}")
        else:
            # Normal KV caching mode: infer position from cache size
            current_size = 0
            if past_kvs is not None:
                current_size = past_kvs[0][0].shape[2]
            pos = torch.arange(current_size, current_size + t, dtype=torch.long, device=device)
            if self.debug:
                print(f"[GPT FORWARD] Using current_size={current_size}, pos={pos.tolist()}")
    ...
    @torch.no_grad()
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None, initial_past_kvs=None):
        """
        ...
        """
        # Initialize past_kvs
        past_kvs = initial_past_kvs
        
        # CRITICAL FIX: Truncate initial cache to exclude last token
        # This is needed because we'll recompute the last token to get its logits
        if initial_past_kvs is not None:
            past_kvs = [(k[:, :, :-1, :], v[:, :, :-1, :]) for k, v in initial_past_kvs]
            if self.debug:
                print(f"[GENERATE] Truncated initial cache from {initial_past_kvs[0][0].shape[2]} to {past_kvs[0][0].shape[2]} tokens")
        ...
        if self.debug:
            ...
            if initial_past_kvs is not None:
                print(f"[GENERATE] Starting with pre-computed KV cache: {initial_past_kvs[0][0].shape}")
                print(f"[GENERATE] After truncation: {past_kvs[0][0].shape}")
            print(f"{'='*80}")
